...
# ============ CONFIGURATION ============
TOP3_RATIO = 0.80   # Minimum ratio for the top 3 featured games
...
MIN_REVIEWS = 2000  # Minimum total reviews to be considered
...
SEARCH_URL = (
    ...
    "323922,29482,1752,4175,1671,1662,1676,791774,493,1770,353880,597,1718,1645,"
    "6506,1754"
    "&category1=998"
    "&category3=2"
...
def main():
    ...
    # Step 2: Fetch reviews and build filtered pool based on MIN_RATIO
    ...
    # Step 3: Sort full pool by weighted score
    games_with_reviews.sort(key=lambda x: x["weighted_score"], reverse=True)

    # Step 4: Pick top 3 from games that also meet TOP3_RATIO
    top3 = [g for g in games_with_reviews if g["ratio"] >= TOP3_RATIO][:3]
    top3_appids = {g["appid"] for g in top3}

    # Step 5: Fill remaining slots from the MIN_RATIO pool, excluding top 3
    remaining_slots = TOP_N_GAMES - len(top3)
    rest = [g for g in games_with_reviews if g["appid"] not in top3_appids][:remaining_slots]

    top_games = top3 + rest

    # Step 6: Fetch Cover URLs
    print(f"\n🖼️  Fetching cover art for Top {len(top_games)} games...")
    ...
    print(f"\nTop 3 (ratio ≥ {TOP3_RATIO:.0%}):")
    for i, game in enumerate(top3, 1):
        print(f"  {i}. {game['name']}: {game['ratio']:.1%} ({game['total_reviews']} reviews) → score: {game['weighted_score']:.0f}")
    print(f"\nRemaining {len(rest)} (ratio ≥ {MIN_RATIO:.0%}):")
    for i, game in enumerate(rest, 1):
        print(f"  {i}. {game['name']}: {game['ratio']:.1%} ({game['total_reviews']} reviews) → score: {game['weighted_score']:.0f}")
